...
int main(void)
{
    ...
    while(1)
    {
        ...
        if(g_save_flag)
        {
            char *des = (uint32_t *)iomem_malloc(8);
            strcpy(des, "0:photo");
            // char *des = "0:photo";
            sprintf(str, "%d", countf);
            strcat(des, str);
            strcat(des, front);
            printf("create file %s \n", des);
            rgb565tobmp((uint8_t *)(g_ram_mux ? g_lcd_gram0 : g_lcd_gram1), 320, 240, _T(des));
            iomem_free(des);
            g_save_flag = 0;
            countf++;
        }
